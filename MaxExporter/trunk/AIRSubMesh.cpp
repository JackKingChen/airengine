...
void CMeshKeyFrameData::SetVerticesNum(int nNum)
{
...
CAIRVertex* CAIRSubMesh::GetVertex(int nIndex)
{
...
bool CAIRSubMesh::SaveToFile(CAIRMesh* pMesh, FILE* f)
{
    ...
    if (!m_bVertexAnimate)
    {
        ...
        {
            CAIRVertex* pVertex = m_vertices[i];
            for (int j = 0; j < pVertex->GetInfluenceNum(); j++)
            {
...
{
    if (m_vtAnimationDatas.size() > nFrame)
    {
        return m_vtAnimationDatas[nFrame];
    }
