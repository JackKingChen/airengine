...
#ifdef AW_EXPORT
#define _XEVOL_BASE_API_ __attribute__((visibility("default")))
#else
#define _XEVOL_BASE_API_
#endif

#define BEGIN_NAMESPACE_XEVOL3D namespace AIR{
...
_XEVOL_BASE_API_  float      wtof_x       (const wchar_t * buffer);
_XEVOL_BASE_API_  int        wtoi_x       (const wchar_t * buffer);
...
_XEVOL_BASE_API_  int        format_value (wchar_t *buffer,size_t count, const wchar_t *fmt , ... );
_XEVOL_BASE_API_  int        vsnwprintf_x(wchar_t* _out , size_t count , const wchar_t *buf , va_list ap);

#define  swscanf_x(str , strFmt , ...) \
do \
{\
    char  _buf[1024] = {0};\
    std::ds_string ansiStr = XW2A_S(str   , _buf , 1024);\
    std::ds_string ansiFmt = XW2A_S(strFmt, _buf , 1024);\
...
{\
    char  _buf[128] = {0};\
    std::ds_string ansiStr = XW2A_S(str   , _buf , 128);\
    std::ds_string ansiFmt = XW2A_S(strFmt, _buf , 128);\
    sscanf(ansiStr.c_str() , ansiFmt.c_str() , __VA_ARGS__ );\
} while (0);\
...
_XEVOL_BASE_API_ const wchar_t*   XEvol_LocaleToUnicode(const char* _in);
_XEVOL_BASE_API_ const char*      XEvol_UnicodeToLocale(const wchar_t* _in);
...
_XEVOL_BASE_API_ const char*    XW2A_S(const wchar_t* _in, char* _out,size_t outlen);
_XEVOL_BASE_API_ const wchar_t* XA2W_S(const char* _in, wchar_t* _out,size_t outlen);
