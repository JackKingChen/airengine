...
const EventTypeSpec UpSpec =
{
    kEventClassKeyboard,
    kEventRawKeyUp
};

const EventTypeSpec ModSpec =
{
    kEventClassKeyboard,
    kEventRawKeyModifiersChanged
};

const EventTypeSpec AllSpecs[] =
{
...
void MacKeyboard::_initialize()
{
	...
	if (status != noErr)
		AIS_EXCEPT( E_General, "MacKeyboard::_initialize >> Error loading KeyDown event handler" );

	if (InstallEventHandler( event, keyUpUPP, 1, &UpSpec, this, &keyUpEventRef ) != noErr)
		AIS_EXCEPT( E_General, "MacKeyboard::_initialize >> Error loading KeyUp event handler" );

	if (InstallEventHandler( event, keyModUPP, 1, &ModSpec, this, &keyModEventRef ) != noErr )
		AIS_EXCEPT( E_General, "MacKeyboard::_initialize >> Error loading Keymods event handler" );
}
...
std::string& MacKeyboard::getAsString( KeyCode key )
{
    ...
    for(VirtualtoAIS_KeyMap::iterator it = keyConversion.begin(); it != keyConversion.end(); ++it)
    {
        if(it->second == key)
...
void MacKeyboard::injectEvent( KeyCode kc, unsigned int time, MacEventType type, unsigned int txt )
{
    ...
    unsigned int keyType = ET_KeyUp;
    if ( type == MAC_KEYDOWN || type == MAC_KEYREPEAT )
    {
        keyType = ET_KeyDown;
    }
	
	if ( mBuffered && mListener )
		pendingEvents.push_back( MacKeyStackEvent( AIS::KeyEvent(keyType, this, kc, txt), type) );
}
