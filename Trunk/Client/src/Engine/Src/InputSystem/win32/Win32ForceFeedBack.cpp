...
#define AIS_WIN32_JOYFF_DEBUG 1

#if (defined (_DEBUG) || defined(AIS_WIN32_JOYFF_DEBUG))
  #include <iostream>
  #include <sstream>
  using namespace std;
#endif
...
unsigned short Win32ForceFeedback::getFFMemoryLoad()
{
	...
	if(FAILED(hr))
	{
		...
		{
		    AIS_EXCEPT(E_General, "Can't query FF memory load as device was not acquired in exclusive mode");
		}
		else
		{
		    AIS_EXCEPT(E_General, "Unknown error querying FF memory load ->..");
		}
	}
...
void Win32ForceFeedback::upload( const Effect* effect )
{
	switch( effect->force )
	{
		case AIS::Effect::ConstantForce: _updateConstantEffect(effect);	break;
		case AIS::Effect::RampForce: _updateRampEffect(effect);	break;
		case AIS::Effect::PeriodicForce: _updatePeriodicEffect(effect);	break;
		case AIS::Effect::ConditionalForce:	_updateConditionalEffect(effect); break;
		//case AIS::Effect::CustomForce: _updateCustomEffect(effect); break;
		default: AIS_EXCEPT(E_NotImplemented, "Requested Force not Implemented yet, sorry!"); break;
	}
}
...
void Win32ForceFeedback::_updatePeriodicEffect( const Effect* effect )
{
	...
	switch( effect->type )
	{
	case AIS::Effect::Square: _upload(GUID_Square, &diEffect, effect); break;
	case AIS::Effect::Triangle: _upload(GUID_Triangle, &diEffect, effect); break;
	case AIS::Effect::Sine: _upload(GUID_Sine, &diEffect, effect); break;
	case AIS::Effect::SawToothUp: _upload(GUID_SawtoothUp, &diEffect, effect); break;
	case AIS::Effect::SawToothDown:	_upload(GUID_SawtoothDown, &diEffect, effect); break;
	default: break;
	}
}
...
void Win32ForceFeedback::_updateConditionalEffect( const Effect* effect )
{
	...
	switch( effect->type )
	{
	case AIS::Effect::Friction:	_upload(GUID_Friction, &diEffect, effect); break;
	case AIS::Effect::Damper: _upload(GUID_Damper, &diEffect, effect); break;
	case AIS::Effect::Inertia: _upload(GUID_Inertia, &diEffect, effect); break;
	case AIS::Effect::Spring: _upload(GUID_Spring, &diEffect, effect); break;
	default: break;
	}
}
...
void Win32ForceFeedback::_upload( GUID guid, DIEFFECT* diEffect, const Effect* effect)
{
	...
	if( dxEffect == 0 )
	{
		...
		{
			AIS_EXCEPT(E_DeviceFull, "Remove an effect before adding more!");
		}
		else
		{
			AIS_EXCEPT(E_General, "Unknown error creating effect->..");
		}
	}
	else
	{
		...
		if(FAILED(hr)) AIS_EXCEPT(E_InvalidParam, "Error updating device!");
	}
}
...
void Win32ForceFeedback::_addEffectSupport( LPCDIEFFECTINFO pdei )
{
	...
	//Determine what the effect type is and how it corresponds to our AIS's Enums
	...
	//	   << pdei->tszName;
	//	AIS_EXCEPT( E_General, ss.str().c_str());
	//}
#endif
}
