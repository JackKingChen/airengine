...
void Win32JoyStick::_initialize()
{
    ...
    else
    {
	    ...
	    if(FAILED(mDirectInput->CreateDevice(mJoyInfo.deviceID, &mJoyStick, NULL)))
		    AIS_EXCEPT( E_General, "Win32JoyStick::_initialize() >> Could not initialize joy device!");

	    if(FAILED(mJoyStick->SetDataFormat(&c_dfDIJoystick2)))
		    AIS_EXCEPT( E_General, "Win32JoyStick::_initialize() >> data format error!");

	    HWND hwin = ((Win32InputManager*)mCreator)->getWindowHandle();

	    if(FAILED(mJoyStick->SetCooperativeLevel( hwin, mCoopSetting)))
		    AIS_EXCEPT( E_General, "Win32JoyStick::_initialize() >> failed to set cooperation level!");

	    if( FAILED(mJoyStick->SetProperty(DIPROP_BUFFERSIZE, &dipdw.diph)) )
		    AIS_EXCEPT( E_General, "Win32Mouse::Win32Mouse >> Failed to set buffer size property" );
...
void Win32JoyStick::_enumerate()
{
    ...
    else
    {
		...
		if( FAILED(mJoyStick->GetCapabilities(&mDIJoyCaps)) )
			AIS_EXCEPT( E_General, "Win32JoyStick::_enumerate >> Failed to get capabilities" );
...
BOOL CALLBACK Win32JoyStick::DIEnumDeviceObjectsCallback(LPCDIDEVICEOBJECTINSTANCE lpddoi, LPVOID pvRef)
{
	...
	diptr.uData             = 0x13130000 | _this->mAxisNumber;
	...
	if (FAILED(_this->mJoyStick->SetProperty(DIPROP_RANGE, &diprg.diph)))
		AIS_EXCEPT( E_General, "Win32JoyStick::_DIEnumDeviceObjectsCallback >> Failed to set min/max range property" );
...
void Win32JoyStick::capture()
{
	...
	for(unsigned int i = 0; i < entries; ++i)
	{
		...
		default:
			...
			{	//If it was nothing else, might be axis enumerated earlier (determined by magic number)
				...
				{
					mState.mAxes[axis].m_AbsVal = diBuff[i].dwData;
					axisMoved[axis] = true;
				}
			}
	...
	if( mBuffered && mListener && entries > 0 )
	{
		...
		for( int i = 0; i < 24; ++i )
		{
			if( axisMoved[i] )
			{
				if( mListener->axisMoved( JoyStickEvent(ET_JoyStickAxisMove, this, mState, i) ) == false )
					return;
			}
		}
		...
		for( int i = 0; i < 4; ++i )
		{
			if( sliderMoved[i] )
			{
				if( mListener->sliderMoved( JoyStickEvent(ET_JoyStickSliderMove, this, mState, i) ) == false )
					return;
			}
		}
	}
...
void Win32JoyStick::captureXInput()
{
	...
	mState.mAxes[0].m_RelVal = value - mState.mAxes[0].m_AbsVal;
	mState.mAxes[0].m_AbsVal = value;
	if(mState.mAxes[0].m_RelVal != 0)
        axisMoved[0] = true;

	//LeftX
    mState.mAxes[1].m_RelVal = inputState.Gamepad.sThumbLX - mState.mAxes[1].m_AbsVal;
    mState.mAxes[1].m_AbsVal = inputState.Gamepad.sThumbLX;

	if(mState.mAxes[1].m_RelVal != 0)
        axisMoved[1] = true;

	//RightY
	value = -(int)inputState.Gamepad.sThumbRY;           
    mState.mAxes[2].m_RelVal = value - mState.mAxes[2].m_AbsVal;
    mState.mAxes[2].m_AbsVal = value;
	if(mState.mAxes[2].m_RelVal != 0)
        axisMoved[2] = true;

	//RightX
    mState.mAxes[3].m_RelVal = inputState.Gamepad.sThumbRX - mState.mAxes[3].m_AbsVal;
    mState.mAxes[3].m_AbsVal = inputState.Gamepad.sThumbRX;
	if(mState.mAxes[3].m_RelVal != 0)
		axisMoved[3] = true;
    ...
    mState.mAxes[4].m_RelVal = value - mState.mAxes[4].m_AbsVal;
    mState.mAxes[4].m_AbsVal = value;
	if(mState.mAxes[4].m_RelVal != 0)
		axisMoved[4] = true;
	...
	mState.mAxes[5].m_RelVal = value - mState.mAxes[5].m_AbsVal;
    mState.mAxes[5].m_AbsVal = value;
	if(mState.mAxes[5].m_RelVal != 0)
		axisMoved[5] = true;
...
bool Win32JoyStick::_doButtonClick( int button, DIDEVICEOBJECTDATA& di )
{
	if( di.dwData & 0x80 )
	{
		...
		if( mBuffered && mListener )
			return mListener->buttonPressed( JoyStickEvent( ET_JoyStickButtonDown, this, mState, button ) );
	}
	else
	{
		mState.mButtons[button] = false;
		if( mBuffered && mListener )
			return mListener->buttonReleased( JoyStickEvent( ET_JoyStickButtonUp, this, mState, button ) );
	}
...
bool Win32JoyStick::_changePOV( int pov, DIDEVICEOBJECTDATA& di )
{
	...
	if( mBuffered && mListener )
		return mListener->povMoved( JoyStickEvent( ET_JoyStickPovMove, this, mState, pov ) );
